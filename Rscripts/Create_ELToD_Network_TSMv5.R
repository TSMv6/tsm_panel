...
sf_walked <- sf_walked[, shp_length := st_length(geometry) ]            # use geometry if format is shp
sf_walked <- sf_walked[, DIST := as.numeric(shp_length * 0.000621371) ] # meters to miles

fields_to_exist <- c("A", "B", "DIST",  "FACTYPE", "AREATYPE", "POSTSPEED", "NLANES", "TOLL", 'NSignals')
if(keep_Counts) fields_to_exist <- c(fields_to_exist, "COUNT", "COUNT_TRK")
if(keep_reverse_flex_lanes) fields_to_exist <- c(fields_to_exist, "REVERSE", "FLEX")

# if(keep_Counts){
#   if(year == 21) fields_to_exist <- c(fields_to_exist, "Count_21")
#   if(year == 22) fields_to_exist <- c(fields_to_exist, "Count_22")
# }
...
dt_link <- dt_link[, SPEED := fcase((FNAME == "Limited" & ANAME == "5_Rural" ) | FNAME ==  "Tolls"  , pmin(0.81 * POSTSPEED + 26.5, 1.10 * POSTSPEED), 
                                    FNAME %in% c("Limited", "Div-Arterial", "Undiv-Arterial") & ANAME != "5_Rural" , 0.85 * POSTSPEED,
                                    FNAME =="Ramps" & DIST <= 0.5, pmax(25, pmin(0.8 * POSTSPEED, POSTSPEED - 15)),
                                    FNAME =="Ramps" & DIST > 0.5 & DIST <= 1,  pmax(25, pmin(0.9 * POSTSPEED, POSTSPEED - 10)), 
                                    FNAME =="Ramps" & DIST > 1 & ANAME != "5_Rural",  pmax(25, 0.9 * POSTSPEED),
                                    ...
                                    POSTSPEED > 35 & !(FNAME %in% c("Div-Arterial", "Limited") & ANAME == "5_Rural") , POSTSPEED * 0.80,
                                    POSTSPEED > 35 & FNAME == "Div-Arterial" & ANAME == "5_Rural", POSTSPEED * 0.90,
                                    ANAME %in% c("1_CBD", "2_Fringe")  , POSTSPEED * 0.8,
                                    ANAME %in% c("3_Residential","4_OBD")  , POSTSPEED * 0.85,
                                    FNAME == "Other" | ANAME == "5_Rural",  1.0 * POSTSPEED)]

# dt_link <- dt_link[is.na(SPEED),  SPEED := POSTSPEED, ]
dt_link <- dt_link[A <= 10491 | B <= 10491, SPEED := 10]
...
# Keep a shape file
sf_walked_t    <- sf_walked[, c( "DIST", "POSTSPEED", "NLANES", "AREATYPE", "TOLL") := NULL]
if(keep_Counts) sf_walked_t  <- sf_walked_t[,  COUNT := NULL]
if(keep_reverse_flex_lanes) sf_walked_t  <- sf_walked_t[,  c("REVERSE", "FLEX") := NULL]

dt_geo_lnk     <- merge(sf_walked_t, df_link2, by = c("A", "B"), all.x = TRUE)
st_write(st_as_sf(setDF(dt_geo_lnk)), out_eltod_shp, append = FALSE)
