# PopSyn — population synthesis

PopSyn builds a **synthetic population** of households and persons that
...
PopSyn, which used a **simultaneous balancer (Newton–Raphson)**. In
...
PopSyn is run twice — once for households (HH) and once for Group Quarters
...
separate utility; the incremental run then applies PopSyn to just those
