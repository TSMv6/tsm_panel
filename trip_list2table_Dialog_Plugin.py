...
"""
    ...
    def _msr_sizeterm_tomls(self):
        """The four size-term TOMLs for msr, read in place (no duplication): the
        canonical SDT/LDT model coefficients bundled in the plugin config, plus the
        MSR truck terms. Each is overridable by a Config key to point at a
        calibrated set without editing the plugin."""
        settings = Config()
        cfg = os.path.join(settings.get("plugin_dir"), "config").replace("\\", "/")
        return {
            "sdt_resident": settings.get("sizeterm_sdt_res") or f"{cfg}/sdt_resident_coefficients/size_terms.toml",
            "sdt_visitor":  settings.get("sizeterm_sdt_vis") or f"{cfg}/sdt_visitor_coefficients/vis_tour_dest.toml",
            "ldt":          settings.get("sizeterm_ldt")     or f"{cfg}/ldt_coefficients_toml/dest_choice.toml",
            "truck":        settings.get("sizeterm_truck")   or f"{cfg}/msr/truck_size_terms.toml",
        }

    def _write_msr_control(self, out_path, trip_list, subarea, lookup, msr_out, hourly_out):
        """Write the `msr disaggregate` control file (TOML) from the panel inputs:
        bundled size-term TOMLs, the 26k subzone land use (MSR lookup), the subarea
        parent-zone list, the TSM-resolution trip list, and the MSR outputs."""
        settings = Config()
        tomls = self._msr_sizeterm_tomls()
        ...
        lines = [
            "# MSR disaggregate control -- generated by the TSM panel (trip list -> table).",
            "[size_terms]",
    ...
    def _run_msr_if_enabled(self, scenarioDir, trip_list_path):
        ...
        Disaggregates the TSM-resolution trip list to the MSR subarea subzones via
        `msr disaggregate <control.toml>`, writing the MSR trip list (with
        O_MSR/D_MSR) + the hourly MSR OD trip table. Returns True if OK / skipped."""
        if not self.groupBox_MSR.isChecked():
            return True
        settings = Config()
        msr_exe = settings.app_exe("utilities/msr.exe")
        if not os.path.exists(msr_exe):
            QMessageBox.critical(self, "Error", f"msr.exe not found at: {msr_exe}")
            return False

        subarea = self.lineEdit_MSRSubarea.text().strip()
        lookup = self.lineEdit_MSRLookup.text().strip()
        if not (subarea and os.path.exists(subarea)):
            QMessageBox.critical(self, "Error", "MSR is enabled but the subarea zone file is missing or not set.")
            return False
        if not (lookup and os.path.exists(lookup)):
            QMessageBox.critical(self, "Error", "MSR is enabled but the subzone lookup (26k land use) is missing or not set.")
            return False
        if not (trip_list_path and os.path.exists(trip_list_path)):
            QMessageBox.critical(self, "Error", f"MSR needs the trip list, not found:\n{trip_list_path}")
            return False

        scen = scenarioDir.replace("\\", "/")
        msr_out    = f"{scen}/subarea_msr_triplist.csv.gz"
        hourly_out = f"{scen}/ELToD_MSR_Hourly_tt.csv"
        control = os.path.join(scenarioDir, "msr_control.toml")
        try:
            self._write_msr_control(control, trip_list_path, subarea, lookup, msr_out, hourly_out)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error writing MSR control file: {e}")
            return False

        try:
            r = subprocess.run([msr_exe, "disaggregate", control],
                               env=Config().app_env(msr_exe),
